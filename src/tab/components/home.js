...
class HomePage {
    ...
    for (let i = 0; i < seriesList.length; i++) {
      ...
      for (const v of s.videos) {
        if ((v.watched || v.progress > 0) && v.watchedAt) {
          if (!lastWatched || v.watchedAt > lastWatched.watchedAt)
            lastWatched = v;
        }
      }
      if (!lastWatched) {
  ...
  renderRow(title, seriesList, onClick) {
    ...
    for (let i = 0; i < seriesList.length; i++) {
      const card = this._createCard(seriesList[i], onClick, i);
  ...
  renderSearchPlaylists(playlists, onAdd, title) {
    ...
    for (let i = 0; i < playlists.length; i++) {
      ...
      card.className = "series-card search-playlist-card card-reveal";
        ...
        try {
          ...
          if (playlists && playlists.length > 0) {
            ...
            for (let pi = 0; pi < playlists.length; pi++) {
              ...
              plCard.className =
                "series-card channel-playlist-card card-reveal";
              plCard.style.setProperty("--i", pi);
  ...
  _createCard(series, onClick, index = 0) {
    ...
    if (inProgress) {
      ...
      series.videos.every((v) => v.watched)
    ) {
      progress = 100;
    }
    const isComplete =
      series.completed ||
      (series.videos.length > 0 && watchedCount === series.videos.length);
    const hasNew = series.newEpisodesCount > 0;
    const previewVideo = this._getPreviewVideo(series);

    const card = document.createElement("div");
    card.className = "series-card card-reveal";
    ...
    playBtn.addEventListener("click", (e) => {
      ...
      if (previewVideo) {
        window.open(
          `https://www.youtube.com/watch?v=${previewVideo.id}&list=${series.playlistId}`,
          "_blank",
          "noopener",
        );
      }
